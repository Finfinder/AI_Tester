...
2. (Optional) Build Docker image:
   ...
   ```bash
   python harness.py # This calls the build_image function
   ```
...
```python
from ai_tester_v2.orchestrator.orchestrator import Orchestrator
...
try:
    ...
    # Save result to ai_tester_report.json
...
- `ai_tester_v2.orchestrator.orchestrator.Orchestrator`: Main class for running the workflow.
- `ai_tester_v2.orchestrator.models.Task`: Data model for defining tasks.
- `ai_tester_v2.orchestrator.models.TaskResult`: Data model for task outcomes.
